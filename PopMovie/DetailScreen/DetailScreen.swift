...
//        self.favControl = (userDefaults.defaults.object(forKey: "") != nil)
        ...
        self.favControl = UserDefault.shared.favorites.contains(movie._id) ? true : false
    ...
    @objc func didTapButton() {
       ...
       // favControl.toggle()
        DispatchQueue.main.async { [self] in
            if((self.favControl == false) && (UserDefault.shared.favorites.contains(movie._id) == false)) {
                //MARK: Verileri kaydetme işlemi
                
                UserDefault.shared.saveData(id: self.movie._id)
                
                self.navigationItem.rightBarButtonItem = UIBarButtonItem(image:  UIImage(systemName: "star.fill"), style: .plain, target: self, action: #selector(self.didTapButton))
            }
            else {
                //MARK: Silme işlemi burada yapılacak
                UserDefault.shared.deleteData(id: self.movie._id)
                print("boş")
